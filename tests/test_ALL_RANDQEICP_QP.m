...
for d=range_d
    for n=range_n
        ...
        if testdca
            ...
            dcf=dcfunc;
            dcf.f=@(x,opt)fobj_eval_QP(x,G,opt);
            ...
            save(sprintf('RESULT//DCA_QP_%s.mat',fname),'xopt','fopt','cputime','iters','lambda','w','err','mu');
        ...
        if testbdca
            ...
            save(sprintf('RESULT//BDCA_QP_%s.mat',fname),'xopt','fopt','cputime','iters','lambda','w','err','mu');
        ...
        if testknitro
            ...
            fobj=@(x)-x'*G*x;
            nlcon=@(x)deal(x'*D*x-1,0);
            Aeq=[];
            beq=[];
            lb=zeros(2*n,1);
            ub=[];
            tic
            [xopt,fopt,exitflag,info]=knitromatlab(fobj,z0,[],[],Aeq,beq,lb,ub,nlcon,[],optimset('Display','off'));
            ...
            save(sprintf('RESULT//KNITRO_QP_%s.mat',fname),'xopt','fopt','cputime','iters','lambda','w','err','mu');
        end
        ...
        if testfiltersd
            ...
            [xopt,fopt,exitflag,info] = filtersd(fobj,gradfobj,z0,lb,ub,nlcon,nljac,1,1);
            ...
            save(sprintf('RESULT//FILTERSD_QP_%s.mat',fname),'xopt','fopt','cputime','iters','lambda','w','err','mu');
        end
        ...
        if testfmincon
            ...
            ub=[];
            tic
            [xopt,fopt,exitflag,info]=fmincon(fobj,z0,[],[],Aeq,beq,lb,ub,nlcon,optimset('Display','off','MaxFunEvals',1e+6));
            ...
            save(sprintf('RESULT//FMINCON_QP_%s.mat',fname),'xopt','fopt','cputime','iters','lambda','w','err','mu');
        end
    end
end
